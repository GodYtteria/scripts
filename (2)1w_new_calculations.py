...
class TA:
    ...
    def RSI(self, close):
        ...
        for i in range(len(rsi)):
            # Assigning scores based on RSI levels and crossovers with moving average
            if rsi.iloc[i] > self.RSIupper:
                rsi_score.iloc[i] = 1
            if rsi.iloc[i] < self.RSIlower:
                rsi_score.iloc[i] = -1
...
df = pd.read_csv('1w_crypto_data.csv')
...
df.to_csv('1w_Crypto_Monitor_List.csv', index=False)
